...
- **Set banner image...** Markdown documents, in any mode. Opens a search box; type a word, press Enter, and click a result to set it as the note's banner image (it writes the `banner:` frontmatter for you). It searches Wikimedia Commons by default, falls back to Openverse if Commons is unreachable, and searches Unsplash instead if you have set an Access Key. See Set banner image in [03-editing.md](03-editing.md).
