...
- **Print.** For any file with content, whatever view you are in. Clicking it prints the normal way; hovering it opens the two sheets. See [Printing](03-editing.md#printing).
  ...
  - **Black and white...** The same document on a plain sheet with none of the palette on it. Rules, borders, ticks and underlines are all still drawn, and a code listing keeps its highlighting in weight and slope. **Ctrl+Shift+P** goes straight to it without the menu.
