...
<div align="center"><img src="images/context-menu.png" width="640" alt="The right-click menu in Reading mode, showing Copy, Set banner image, Print / Save as PDF, View as hex, Select all, Zoom and Keyboard shortcuts"></div>
...
- **Set banner image...** Markdown documents, in any mode. Opens a search box; type a word, press Enter, and click a result to set it as the note's banner image (it writes the `banner:` frontmatter for you). It searches Wikimedia Commons by default and falls back to Openverse if Commons is unreachable. Set an Unsplash Access Key (and optionally an Openverse key) in Settings to add those as tabs you can switch between. See Set banner image in [03-editing.md](03-editing.md).
...
- **Code and text** files get a simpler Copy (**Selection** and **Entire document**), plus Find / Replace and Go to line in their Source view (the minified/binary fallback view has neither, since no editor is mounted there). No Insert and no Print entry. LaTeX / TeX source counts as prose-like for maths, so it gets the Math actions, but not Insert image (which writes Markdown image syntax).
...
- Below the grid actions you still get **Print / Save as PDF...**, **View as hex** (when the Binary inspector setting is on), **Select all** (which here selects every grid cell, ready to copy, rather than the page text), **Zoom**, and **Keyboard shortcuts…**.
...
<div align="center"><img src="images/save-menu.png" width="320" alt="The Save split button with its drop-down open: Save As, Save a Copy, Export as HTML and Print / Save as PDF"></div>
